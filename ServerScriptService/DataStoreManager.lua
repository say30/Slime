...
-- CONFIGURATION
-- ========================================
local DATA_STORE_NAME = "SlimeRushData_V1" -- Incrémenter si structure change
...
-- Cache local des données joueurs
...
function DataStoreManager.LoadData(player)
    ...
    if success then
        if data then
            -- Données existantes : fusionner avec structure par défaut
            ...
            if DataManager.ValidateData(data) then
                ...
                warn(string.format("[DataStore] ⚠️ Données corrompues pour %s, création nouvelles données", player.Name))
        ...
        -- Échec chargement : données temporaires
        warn(string.format("[DataStore] ❌ Échec chargement pour %s, données temporaires", player.Name))
...
function DataStoreManager.SaveData(player, forceImmediate)
    ...
    if not data then
        warn(string.format("[DataStore] ⚠️ Aucune donnée à sauvegarder pour %s", player.Name))
        return false
    end
    ...
    if forceImmediate then
        ...
        if success then
            print(string.format("[DataStore] ✅ Sauvegarde immédiate réussie pour %s", player.Name))
            return true
        else
            warn(string.format("[DataStore] ❌ Échec sauvegarde pour %s: %s", player.Name, tostring(err)))
            return false
        end
    else
        -- Ajouter à la queue
...
-- ========================================
-- METTRE À JOUR DONNÉES EN CACHE
-- ========================================
...
spawn(function()
    while true do
        ...
        print("[DataStore] 🔄 Auto-save en cours...")
        ProcessSaveQueue()

        -- Sauvegarder aussi tous les joueurs connectés
...
game:BindToClose(function()
    print("[DataStore] 🛑 Serveur fermeture - Sauvegarde tous les joueurs...")

    for _, player in ipairs(Players:GetPlayers()) do
        DataStoreManager.SaveData(player, true)
    end

    wait(2) -- Attendre les sauvegardes
    print("[DataStore] ✅ Toutes les sauvegardes terminées")
end)
